...
第 1、2、4、8、11 条已实施（保留划线记录）；其余为「已识别但未做」，当前实现是正确的、只是不够省：
...
4. ~~**`summarizeExpenses` 不是单快照**~~ —— **已实施**：三条语句（合计 / 分类明细 / 按人汇总）包进 `withReadTransaction`（`BEGIN DEFERRED`），并发写入落库时不再出现「合计与分类明细对不上」；已在事务内时复用外层快照，不嵌套 BEGIN。
