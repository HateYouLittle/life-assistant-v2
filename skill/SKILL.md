...
- **这些是服务端自动推的，不用手动查**：每日 07:00 天气简报（含补班/放假提示行）、气象预警巡检（每 20 分钟，同预警同级别只推一次、**级别升级会再推**；静默时段内失效的预警不再补投，`notify.pull` 里还能看到）、调休/补班提醒（当天补班、假期前 3 天放假安排、假期最后一天恢复上班）、每月 1 号 09:00 月报。
